...
fatura_model <- fatura_model %>% mutate(TARIH=as.Date(TARIH), MUSTERINO = as.factor(MUSTERINO))

#--------------------------referans tarih -> refday----------------------------------------------------------------------------
refday <- max(fatura_model$TARIH)
...
#---------------------------------------session kayedilmesi-------------------------------------------------
save.image('/Users/sdrttnclskn/Desktop/makale/r_kodlar/ftoplamtutar_model.rdata')
